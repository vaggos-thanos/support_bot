...
let modCMDToOwner = false;
class anti_mod {
    constructor(client) {
        this.isBotOwner = functions.isAuthor
        this.client = client
    }
    ...
    async disable_bot(interaction) {
        try {
            functions.log(`${ansiColors.yellow("[Anti-Mod]")} Command [${interaction.commandName}] in ${interaction.guild.name} Blocked`)
    ...
    async execute(interaction) {
        ...
        functions.log(`${ansiColors.yellow("[Anti-Mod]")} Command [${interaction.commandName}] in ${interaction.guild.name}`)
        ...
        if(await this.check_command(interaction)) {
            ...
            functions.log(`${ansiColors.yellow("[Anti-Mod]")} Command [${interaction.commandName}] in ${interaction.guild.name} tried to ${member}`)
